...
void kt_for(int n_threads, void (*func)(void*,long,int), void *data, long n)
{
	if (n_threads > 1) {
		...
		for (i = 0; i < n_threads; ++i)
			t.w[i].t = &t, t.w[i].i = i;
		#pragma omp parallel for num_threads (n_threads) schedule (dynamic) private(i)
			for (i=0; i<n; i++)
			{
...
//		fprintf (stderr, "start step %d w->index %ld\n", w->step, w->index);
		// working on w->step
		w->data = p->func(p->shared, w->step, w->step? w->data : 0); // for the first step, input is NULL

//		fprintf (stderr, "end step %d w->index %ld\n", w->step, w->index);
		// update step and let other workers know
		pthread_mutex_lock(&p->mutex);
		w->step = w->step == p->n_steps - 1 || w->data? (w->step + 1) % p->n_steps : p->n_steps;
		if (w->step == 0) 
		{
//			fprintf (stderr, "change index %ld to %ld \n", w->index, p->index);
			w->index = p->index++;
		}
