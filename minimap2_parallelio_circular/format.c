...
static void write_sam_cigar(kstring_t *s, int sam_flag, int in_tag, int qlen, const mm_reg1_t *r, int opt_flag, fullRec *fR, otherRec *oR, int *isFr)
{
	if (r->p == 0) {
		mm_sprintf_lite(s, "*");
	} else {
//		printf ("write_sam_cigar ncigars %d\t", r->p->n_cigar);
		...
		if (in_tag) {
			...
			if ((r->p->n_cigar==1) && ((r->p->cigar[0]&0xf)==0) && ((r->p->cigar[0]>>4)==qlen))
			{
				*isFr = 1;
				fR->matchLen = qlen;
			}
			int clip_char = (sam_flag&0x800) && !(opt_flag&MM_F_SOFTCLIP)? 'H' : 'S';
			int clip_num = (clip_char=='H')? 5:4;
			if (clip_len[0]) 
			{
				mm_sprintf_lite(s, "%d%c", clip_len[0], clip_char);
				oR->cigar[oR->n_cigar] = (uint16_t)(clip_len[0]<<4) | clip_num;
				oR->n_cigar++;
			}
			for (k = 0; k < r->p->n_cigar; ++k)
			{
				mm_sprintf_lite(s, "%d%c", r->p->cigar[k]>>4, "MIDN"[r->p->cigar[k]&0xf]);
				if ((oR->n_cigar < 10) && ((r->p->cigar[k]&0xf) < 3)) 
					oR->cigar[oR->n_cigar++] = (uint16_t)r->p->cigar[k];
			}
			if (clip_len[1]) 
			{
				mm_sprintf_lite(s, "%d%c", clip_len[1], clip_char);
				if (oR->n_cigar < 10)
					oR->cigar[oR->n_cigar++] = (uint16_t)(clip_len[1]<<4) | clip_num;
			}
			if (oR->n_cigar > 1) *isFr = 0;
		}
	}
}
...
void mm_write_sam2(kstring_t *s, const mm_idx_t *mi, mm_bseq1_t *t, int seg_idx, int reg_idx, int n_seg, const int *n_regss, const mm_reg1_t *const* regss, void *km, int opt_flag, FILE **aebFp, FILE **aibFp, int numOutChunks, int numMaxChunks, int maxChunkSize, char *prefix, vec_fullRec *vfr, vec_otherRec *vor)
{
	...
	if (r == 0) {
		...
		fR.pos = oR.pos = r->rs+1;
		fR.qual = oR.qual = r->mapq;
		t->contig = r->rid;
		...
		} else write_sam_cigar(s, flag, 0, t->l_seq, r, opt_flag, &fR, &oR, &isFr);
	}
//	if (isFr == 1)
//		printf ("\tAEB%d  cigar_in_tag %d\t", oR.n_cigar, cigar_in_tag);
//	else if (isFr == 0)
//		printf ("\tAIB%d  cigar_in_tag %d\t", oR.n_cigar, cigar_in_tag);
	...
	if (r == 0) {
		...
		for (i1=0; i1<t->l_seq; i1++)
		{
			int seqInd = i1/2;
			int subInd = i1&1;
			if (subInd==0)
			{
				fR.seq[seqInd] = bToInt[seq_nt4_table[(int)t->seq[i1]]]<<4;
				oR.seq[seqInd] = bToInt[seq_nt4_table[(int)t->seq[i1]]]<<4;
			}
			else
			{
				fR.seq[seqInd] |= bToInt[seq_nt4_table[(int)t->seq[i1]]];
				oR.seq[seqInd] |= bToInt[seq_nt4_table[(int)t->seq[i1]]];
			}
		}
		if (t->qual) 
		{
			...
			for (i1=0; i1<t->l_seq; i1++)
			{
				fR.quals[i1] =  t->qual[i1] - 33;
				oR.quals[i1] =  t->qual[i1] - 33;
			}
		}
		...
		if ((flag & 0x900) == 0 || (opt_flag & MM_F_SOFTCLIP)) {
			...
			for (i1=0; i1<t->l_seq; i1++)
			{
				...
				if (subInd==0)
				{
					fR.seq[seqInd] = bToInt[seq_nt4_table[(int)s->s[cur_len+i1]]]<<4;
					oR.seq[seqInd] = bToInt[seq_nt4_table[(int)s->s[cur_len+i1]]]<<4;
//					printf ("%d", bToInt[seq_nt4_table[(int)s->s[i1]]]);
				}
				else
				{
					fR.seq[seqInd] |= bToInt[seq_nt4_table[(int)s->s[cur_len+i1]]];
					oR.seq[seqInd] |= bToInt[seq_nt4_table[(int)s->s[cur_len+i1]]];
//					printf ("%d ", bToInt[seq_nt4_table[(int)s->s[i1]]]);
				}
			}
			...
			if (t->qual) 
			{
				...
				for (i1=0; i1<t->l_seq; i1++)
				{
					fR.quals[i1] = s->s[cur_len+i1] - 33;
					oR.quals[i1] = s->s[cur_len+i1] - 33;
				}
//				memcpy (fR.quals, s->s+cur_len, t->l_seq);
//				memcpy (oR.quals, s->s+cur_len, t->l_seq);
			}
			else mm_sprintf_lite(s, "*");
		} else if (flag & 0x100) {
			mm_sprintf_lite(s, "*\t*");
...
//				printf ("qual len %d\n", s->l);
//				memcpy (fR.quals, s->s+cur_len, r->qe - r->qs);
//				memcpy (oR.quals, s->s+cur_len, r->qe - r->qs);
			}
			else mm_sprintf_lite(s, "*");
		}
	}
	if ((t->contig != -1) && (aebFp != NULL))
	{
		char fName[500];
		if (isFr == 1)
		{
/*			t->fR = malloc (sizeof(fullRec));
			assert (t->fR != NULL);
			t->oR = NULL;
			memcpy (t->fR, &fR, sizeof(fullRec));*/
...
//			fwrite (&fR, sizeof(fullRec), 1, aebFp[fileNo]);
		...
		{
/*			t->fR = NULL;
			t->oR = malloc (sizeof(otherRec));
			assert (t->oR != NULL);
			memcpy (t->oR, &oR, sizeof(otherRec));*/
...
/*	if ((t->fileNo == 0) && (t->contig != 0)) 
	{
		fprintf (stderr, "fileno 0, contig %d(pos %d), maxChunkSize %d, isfr %d, aebfp %p\n", t->contig, fR.pos-1, maxChunkSize, isFr, aebFp);
	}*/
	// write tags
	...
	if (r) {
		...
		if (r->p && (opt_flag & MM_F_OUT_CS))
			write_cs(km, s, mi, t, r, !(opt_flag&MM_F_OUT_CS_LONG));
//		printf ("before cigar_in_tag %d, ncigar %d\n", cigar_in_tag, r->p->n_cigar);
		if (cigar_in_tag)
			write_sam_cigar(s, flag, 1, t->l_seq, r, opt_flag, &fR, &oR, &isFr);
	}
