...
void split_fasta (mm_idx_seq_t *seq, int numContigs, int *numOutChunks, int *numMaxChunks, int *maxChunkSize)
{
        ...
        {
                if (seq[i].len > maxContigSize) maxContigSize = seq[i].len;
        }
        ...
        while (curChunks < numChunks)
        {
                ...
                for (i=0; i<numContigs; i++)
                {
                        curChunks += (seq[i].len/curContigSize)+(seq[i].len%curContigSize > 0);
                }
//                printf ("curChunkSize %d --> numChunks %d\n", curContigSize, curChunks);
        ...
        for (i=0; i<numContigs; i++)
        {
                int curChunksPerContig = (seq[i].len/curContigSize)+(seq[i].len%curContigSize > 0);
...
{
	const char *opt_str = "2aSw:k:K:o:t:r:f:Vv:g:G:I:d:XT:s:x:Hcp:M:n:z:A:B:O:E:m:N:Qu:R:hF:LC:";
	mm_mapopt_t opt;
	mm_idxopt_t ipt;
	int i, c, n_threads = 3, long_idx;
	char *fnw = 0, *rg = 0, *s, *oprefix=NULL;
	FILE *fp_help = stderr;
...
//	assert (provided
	...
	if (argc == optind || fp_help == stdout) {
		...
		fprintf(fp_help, "    -o PREFIX    output prefix path for AEB and AIB output files\n");
