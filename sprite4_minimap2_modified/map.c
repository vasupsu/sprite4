...
//				s->seq[i].fR = s->seq[i].oR = NULL;
		...
		#pragma omp critical (write_lock)
		{
			...
			free(s->buf);
			vec_fullRec *vfr = (vec_fullRec *)calloc (p->numMaxChunks*mi->n_seq, sizeof(vec_fullRec));
			vec_otherRec *vor = (vec_otherRec *)calloc (p->numMaxChunks*mi->n_seq, sizeof(vec_otherRec));
			assert ((vfr != NULL) && (vor != NULL));
	...
	//					puts(p->str.s);
			...
			for (i=0; i<p->numMaxChunks*mi->n_seq; i++)
			{
				if (vfr[i].m > 0) 
				{
					assert (p->aebFp[i] == NULL);
					if (p->aebFp[i] == NULL) {
...
//	int numtasks = 2, rank=0;
	...
	int numChunks = (int)(buf.st_size/sizeof(fq_index))-1;
	FILE *fqidx = fopen (fq_idx_file, "r");
	pl.fI = (fq_index *)malloc ((numChunks+1) * sizeof(fq_index));
	fread (pl.fI, sizeof(fq_index), numChunks+1, fqidx);
/*	for (i = 0; i <= numChunks; ++i) {
		fprintf (stderr, "%d- %lu,%lu\n", i, pl.fI[i].offset, pl.fI[i].offset2);
	}*/
	fclose (fqidx);
...
//			fprintf (stderr, "rank %d seg %d open file %s\n", rank, i, fn[i]);
			if (i==0)
				pl.fp[i] = mm_bseq_open(fn[i], pl.fI[pl.cur_chunk].offset);
			else if (i==1)
				pl.fp[i] = mm_bseq_open(fn[i], pl.fI[pl.cur_chunk].offset2);
			else
				pl.fp[i] = mm_bseq_open(fn[i], 0);
