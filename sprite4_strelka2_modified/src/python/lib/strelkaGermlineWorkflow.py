...
def strelkaGermlineGetDepthFromAlignments(self,taskPrefix="getChromDepth",dependencies=None):
    ...
    if os.path.exists(outputPath):
        print ("strelkaGermlineGetDepthFromAlignments: FILE EXISTS: " + outputPath)
        return set()
    else:
        print ("strelkaGermlineGetDepthFromAlignments: FILE NOT FOUND: " + outputPath)
...
class StrelkaGermlineWorkflow(StrelkaSharedWorkflow) :
    ...
    def workflow(self) :
        ...
        print ("ESTIMATE_PRE_REQS 1: ")
        for tok in estimatePreReqs:
            print (tok)
        if self.params.isHighDepthFilter :
            estimatePreReqs |= strelkaGermlineGetDepthFromAlignments(self)
        print ("ESTIMATE_PRE_REQS 2: ")
        for tok in estimatePreReqs:
            print (tok)
