...
void ActiveRegion::processHaplotypes()
{
    ...
    // bypass haplotyping
//    std::cout << "PRINT processHaplotypes " << isRangeValid << " - " << _posRange.size() << std::endl;
...
//	std::cout << "PRINT processHaplotypes after processHaplotypesWithCounting " << isHaplotypingSuccess << std::endl;
...
//    std::cout << "PRINT processHaplotypesWithCounting\n";
...
//        std::cout << "start " << start << " end " << end << " npos " << std::string::npos << std::endl;
        if (end == std::string::npos or start > end) 
        {
//            std::cout << "PRINT no suffix anchor\n";
            continue;
        }
...
//                std::cout << "PRINT ignoring pseudo read\n";
                continue;
            }
        ...
        if (not containsUniqueRead) 
        {
//            std::cout << "PRINT no uniq read supporting contig\n";
            continue;
        }
//        std::cout << "haplotype " << haplotype << "\nrefStr    " << refStr << std::endl;
        if (haplotype != refStr)
            isNonRefHaplotypeFound = true;
        haplotypeToAlignIdSet[haplotype] = alignIds;
    }

//    std::cout << "PRINT processHaplotypesWithAssembly isNonRefHaplotypeFound " << isNonRefHaplotypeFound << std::endl;
...
{
//    std::cout << "PRINT processSelectedHaplotypes\n";
    ...
    const uint8_t haplotypeId)
{
    ...
    if (reference == haploptypeSeq)
        return;
//    std::cout << "PRINT convertToPrimitiveAlleles\n";
    pos_t referencePos;
    ...
    for (const auto& pathSegment : alignPath)
    {
        ...
        {
//            std::cout << "PRINT convertToPrimitiveAlleles ALIGNPATH::INSERT\n";
...
//                std::cout << "PRINT call addIndelObservation 1\n";
