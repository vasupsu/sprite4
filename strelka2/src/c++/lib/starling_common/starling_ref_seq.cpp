    ...
    std::vector<AnalysisRegionInfo>& regionInfoList)
{
    ...
    int rSize = 300000000;
#ifdef USE_MPI
    int32_t numRanges(0), curRange(0);
    for (unsigned regionIndex(0); regionIndex < regionCount; ++regionIndex)
    {
        int32_t regionBeginPos(0), regionEndPos(0);
        std::string chrom;
        const std::string& region(opt.regions[regionIndex]);
        parse_bam_region(region.c_str(), chrom, regionBeginPos, regionEndPos);
        // check that target region chrom exists in bam headers:
        if (not referenceHeaderInfo.chrom_to_index.count(chrom))
        {
            using namespace illumina::common;
            std::ostringstream oss;
            oss << "ERROR: region contig name: '" << chrom
                << "' is not found in the header of BAM/CRAM file: '" << referenceAlignmentFilename
                << "'\n";
            BOOST_THROW_EXCEPTION(LogicException(oss.str()));
        }
        numRanges += (regionEndPos-regionBeginPos)/rSize;
	if ((regionEndPos-regionBeginPos)%rSize) numRanges++;
    }
    
    regionInfoList.resize(numRanges);
    for (unsigned regionIndex(0); regionIndex < regionCount; ++regionIndex)
    {
        int32_t regionBeginPos(0), regionEndPos(0);
        std::string chrom;
	...
	int32_t lastPos(regionBeginPos);
        for (int32_t posIndex(regionBeginPos+rSize); posIndex <= regionEndPos; posIndex+=rSize)
        {
            auto& regionInfo(regionInfoList[curRange++]);
            getStrelkaAnalysisRegionInfo (chrom, lastPos, posIndex, supplementalRegionBorderSize, regionInfo);
            lastPos = posIndex;
        }
        if (lastPos < regionEndPos)
        {
            auto& regionInfo(regionInfoList[curRange++]);
            getStrelkaAnalysisRegionInfo (chrom, lastPos, regionEndPos, supplementalRegionBorderSize, regionInfo);
        }
    }
#else
